...
% use single dipole simulations as a starting point (the leadfield matrix, patch locations,..)
...
scale = [1 10 1/10 5 1/5 3 1/3 100 0.01]; % scaling factors
save('scaling','scale')
source_recon_idx = 1:3; % change if you want to perform simulations for specific source reconstruction approaches only
        ...
        for sim_i = 1:length(scale)
            for j = rep_idx
                ...
                for s = 1:length(SNR)
                    ...
                    G(:,x) = G(:,x)+G(:,x).*(scale(sim_i)-scale(1)).*repmat(full(weighted_x)',size(G,1),1));
                    load(sprintf('%srep%d_params_%d',myelin_sim_dir_single_dipoles,j,mod(setNr,2)+1),'weighted_x','x')
                    G(:,x) = G(:,x)+G(:,x).*(1./scale(sim_i)-scale(1)).*repmat(full(weighted_x)',size(G,1),1);
                    ...
                    % the perform source reconstruction across forward models with different scaling parameters
                    for i = 1:length(scale) 
                        load('SPMgainmatrix_Sim_MEG_data_coreg_err_0_1.mat')
                        load(sprintf('%srep%d_params_%d',myelin_sim_dir_single_dipoles,j,setNr),'weighted_x','x')
                        G(:,x) = G(:,x)+G(:,x).*(scale(i)-scale(1)).*repmat(full(weighted_x)',size(G,1),1);
                        load(sprintf('%srep%d_params_%d',myelin_sim_dir_single_dipoles,j,mod(setNr,2)+1),'weighted_x','x')
                        G(:,x) = G(:,x)+G(:,x).*(1./scale(i)-scale(1)).*repmat(full(weighted_x)',size(G,1),1);
                        save(sprintf('adapted_SPMgainmatrix_Sim_MEG_data_coreg_err_0_1.mat'),'G','label')

                        rep_data = [myelin_sim_dir '/rep_Sim_MEG_data_coreg_err_0.mat']; 
                        D = spm_eeg_load(rep_data);
                        D.inv{1}.gainmat = 'adapted_SPMgainmatrix_Sim_MEG_data_coreg_err_0_1.mat'; % used gainmatrix with adapted leadfields
                        D.save
